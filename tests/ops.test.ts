...
describe('private /ops PWA',()=>{
 ...
 it('creates a hardened session and recovers expired sessions',async()=>{process.env.OPS_PASSWORD='owner-pass';process.env.OPS_SESSION_SECRET='x'.repeat(32);const a=app();expect((await a.inject({method:'POST',url:'/ops/login',payload:{password:'wrong'}})).statusCode).toBe(401);const login=await a.inject({method:'POST',url:'/ops/login',payload:{password:'owner-pass'}});const set=login.headers['set-cookie'] as string;expect(set).toContain('HttpOnly');expect(set).toContain('Secure');expect(set).toContain('SameSite=Strict');const c=set.split(';')[0]!;const page=await a.inject({url:'/ops',headers:{cookie:c}});expect(page.statusCode).toBe(200);expect(page.body).toContain('Offline snapshot');expect(page.body).toContain('<meta charset="utf-8">');expect(page.body).toContain('PROJECT MANAGEMENT');expect(page.body).toContain('Whole project');expect(page.body).toContain('Product Development');expect(page.body).toContain('EPIC · Living Chart release');expect(page.body).toContain('Package SHA 85fc8249');
    expect(page.body).toContain('PR #9 at aca46ca');
    expect(page.body).toContain('Review the Safe Floor, mastery and module-exam proposal');expect(page.body).toContain('Let two agents build Tikerino side by side');
    expect(page.body).toContain('Review audience, method and consent validation package');expect(page.body).toContain('data-state="Review"');expect((await a.inject({url:'/ops',headers:{cookie:'tikerino_ops=1.invalid'}})).statusCode).toBe(401);expect((await a.inject({url:'/ops/manifest.webmanifest',headers:{cookie:c}})).statusCode).toBe(200);const sw=await a.inject({url:'/ops/sw.js',headers:{cookie:c}});expect(sw.statusCode).toBe(200);expect(sw.headers['service-worker-allowed']).toBe('/ops/');expect(sw.body).toContain("tikerino-ops-v2");expect(sw.body).toContain("caches.delete")});
});
