...
const backup = args.find((a) => a.startsWith('--backup='))?.slice('--backup='.length)?.trim();
const sourceTable = args.find((a) => a.startsWith('--source='))?.slice('--source='.length) ?? 'audit_records';
...
if (!exists.rows[0]?.table) {
  console.error(`No ${sourceTable} table in this database. Nothing to migrate - is this the right DATABASE_URL?`);
  await pool.end();
  process.exit(1);
}

const { rows } = await pool.query(`SELECT record FROM ${sourceTable} ORDER BY id`);
console.log(`Read ${rows.length} row(s) from ${sourceTable}. The source is not modified by this script.`);
...
if (rejected.length > 0) {
  ...
  for (const r of rejected.slice(0, 10)) console.error(`  row ${r.index}: missing ${r.missing.join(', ')}`);
...
console.log(
  `Inserted ${after - before}. Already present, skipped: ${records.length - (after - before)}. Rows in audit_answers: ${after}.`,
);
